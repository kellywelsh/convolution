# -*- coding: utf-8 -*-
"""
Created on Wed May  8 20:01:35 2019
...
def model_predict(data_test, y_label, learning_rate = 0.009, num_epochs = 100, minibatch_size = 10, print_cost = True):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "model_softmax1_2019.ckpt")        
...
img = cv2.imread("Nueve.jpg")

img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
ret,img = cv2.threshold(img,128,255,cv2.THRESH_BINARY_INV)
img = cv2.resize(img, (28, 28), interpolation=cv2.INTER_CUBIC)#
#img = cv2.resize(img[:,:,0], (28, 28), interpolation=cv2.INTER_CUBIC)#
img_test = img[np.newaxis,...,np.newaxis]/255.
plt.imshow(img)
